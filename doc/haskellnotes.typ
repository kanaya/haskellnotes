...
// #footnote[関手は英語でファンクター(functor)と言うが，C++の関数オブジェクト （function object）もかつてはファンクター（functor）と呼ばれていた．C++のファンクターとはクロージャの代用品のことで，本書で述べる関手とは異なる概念である．混同しないように注意しよう．]
...
ここで2引数を取る関数 $g colon.double haskell.a -> haskell.a -> haskell.a$ があるとする．この関数 $g$ をリスト変数 $x_"s"$ の各要素と，同じ長さの別なリスト変数 $y_"s"$ の各要素に適用した結果をリスト変数 $z_"s"$ とする．このことを我々はどのように書いたら良いだろうか．一案はリスト内包表記に戻ることであり，次のように書くことである．
...
一般マップ演算子をピュア演算子と一般アプリカティブマップ演算子に分解することで，式の見通しを良くすることができるアプリカティブスタイルという記法を採用できた．いま変数 $x, y colon.double haskell.a$ があり，引数を文脈に入れる関数 $phi, psi colon.double haskell.Applicative supset haskell.f arrow.r.stroked haskell.a -> haskell.fa$ および $omega colon.double haskell.Applicative supset haskell.f arrow.r.stroked haskell.a -> haskell.a -> haskell.fa$ があるとしよう．すると，アプリカティブスタイルでは次のようにコンテナ変数 $u_*, v_*$ に関数 $omega$ を適用させることができる．
