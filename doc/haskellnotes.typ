...
Haskellでは全ての変数に対して破壊的代入が禁止されている．例外はSTモナド型やIOモナド型の内部で行われる破壊的代入程度である．一方，C言語ではデフォルトで変数への破壊的代入が許可されている．そこで「うっかり」破壊的代入をしないように，変数の型に ```c const``` 修飾子をつけることが出来る．例えば ```main``` 関数の第2引数 ```c argv``` を無意味に破壊しないように ```c const char *const *argv``` と書くことが出来る．
