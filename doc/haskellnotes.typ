...
一般マップ演算子をピュア演算子と一般アプリカティブマップ演算子に分解することで，式の見通しを良くすることができるアプリカティブスタイルという記法を採用できた．関数 $f, g, h$ が引数を文脈に入れる関数であるとしよう．すると，アプリカティブスタイルでは次のようにコンテナ変数 $u_*, v_*, w_*$ に関数 $i$ を適用させることができる．
