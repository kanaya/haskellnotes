...
Maybeとよく似た型にEitherがある．Maybeが $haskell.a$ 型または $\hNothing$ のいずれかの値をとったように，Eitherは $haskell.a$ 型または $haskell.b$ 型のいずれかの値を取る．$haskell.a$ 型または $haskell.b$ 型を取るEither型の変数 $\hEitherVar{e}$ があるとすると，
\begin{equation}
  \hEitherVar{e}
...
\end{equation}
とする．この新しいマップ演算子 $haskell.bindMaybe$ のことをMaybeの#keyword[バインド演算子}と呼ぶ．ここで，もし計算が成功していたら $\hMaybeVar{v}=\hJustWith{\hxFunc{g}\hxVar{x}}$ であり，失敗していたら $\hMaybeVar{v}=\hNothing$ である．
