...
#showybox(title: "ラムダの理由")[我々のラムダ式 $backslash x |-> x + 1$ は，かつては $hat(x) . x + 1$ のように書いていた．その後 $Lambda x . x + 1$ に変化し $lambda x . x + 1$ のように変化したらしい．多くの教科書はこの $lambda$ を使う記法を採用している．$hat(x)$ が $Lambda x$ に変化したのは形が似ているからである．]
