...
である．つまりリストのマップ演算子 $*$ が関数 $f$ を $[haskell.a]$ 世界から $[haskell.b]$ 世界へとジャンプさせ，Maybeのマップ演算子 $ast.op.o_?$ が関数 $f$ を $haskell.a$ 世界から $haskell.MaybeA$ 世界へとジャンプさせているのである．ジャンプを波矢印 $(~~>)$ で表すと，次の表のようになる．
...
いま「世界」と呼んだものを，数学者は#keyword[圏]と呼ぶ．圏とは#keyword[対象]と#keyword[射]の組み合わせである．本書では「対象」とは変数のことであり，射とは関数だと思えば良い．そして，圏から圏へとジャンプさせるもの $(~~>)$ を#keyword[関手]と呼ぶ．この例で言えば値コンストラク $[x], haskell.Just(x)$ とマップ演算子 $*, ast.op.o_?$ が関手である．値コンストラクタ $haskell.Just(x)$ は $haskell.a -> haskell.MaybeA$ という型を持ち，マップ演算子 $ast.op.o_?$ は $(haskell.a -> haskell.b) -> (haskell.MaybeA -> haskell.MaybeB)$ という型を持つ．#footnote[関手は英語でファンクター(functor)と言うが，C++の関数オブジェクト （function object）もかつてはファンクター（functor）と呼ばれていた．C++のファンクターとはクロージャの代用品のことで，本書で述べる関手とは異なる概念である．混同しないように注意しよう．]
...
#par-equation($ (ast.op.o) colon.double haskell.Functor supset haskell.f arrow.r.stroked haskell.f_haskell.a -> haskell.f_haskell.b $)
...
$ id ast.op.o_? x_? &= haskell.kwcase x_? haskell.kwof cases(haskell.Just(x) arrow.r.dotted haskell.Just(id x), haskell.Nothing "otherwise") $
...
リスト型が関手則に従うことを確認しておこう．任意のリスト型変数 $x_"s" colon.double [haskell.a]$ について，関手マップ演算子 $(ast.op.o)$ は次のようになる．
#par-equation($ id ast.op.o x_"s" &= [id x | x in x_"s"] \
  &= [x | x in x_"s"] \
  &= x_"s" $)
これはリストに対しても恒等射が存在することを意味する．

次に関数の結合則が成り立っていることも確認しよう．任意の関数 $f, g colon.double haskell.a -> haskell.a$ について
#par-equation($ (g compose f) ast.op.o x_"s" &= [(g compose f) x | x in x_"s"] \
  &= [g(f x) | x in x_"s"] \
  &= g ast.op.o [f x | x in x_"s"] \
  &= g ast.op.o (f ast.op.o x_"s") $)
であるから，結合則も成り立っている．

Maybe型も関手則に従うことを確認しておこう．Maybe型の変数 $x_? colon.double haskell.MaybeA$ について
#par-equation($ id ast.op.o x_? 
...
$ w_* = omega ast.op.o (psi y) ast.square (phi x) $
