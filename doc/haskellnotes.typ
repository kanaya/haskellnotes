...
#showybox(title: "例外と大域ジャンプ")[多くのプログラミング言語が大域ジャンプをサポートしている．大域ジャンプとは，プログラムのある部分から別の部分へのジャンプを行うことを指す．多くの場合は，プログラムの逐次実行を中断し，ラベルの付けられた別の場所へ飛ぶ機能が提供される．古典的には `goto` 命令とラベルの組み合わせが大域ジャンプに使われる．プログラム中の `goto` 命令には大した害が無いものの，ラベルはプログラムの可読性を著しく低下させる．ラベルだけでは，どこから飛んできたのか判断がつかないからである．そもそも大域ジャンプが必要なのはエラー処理などの「例外的な事象」に限ると割り切って，専用の構文を導入したのがプログラミングにおける「例外」機構である．例外機構によってエラー処理の見通しは良くなるが，例外機構は参照透過性を破壊するため，関数型プログラミング言語では扱いにくい．そのためにMaybeを導入したのがHaskellであり，Maybeの劣化コピーが多くのオブジェクト指向プログラミング言語で採用されている．]
