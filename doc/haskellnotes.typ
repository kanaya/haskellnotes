...
のようにまとめて書くことにしよう．#footnote[Haskellでは ```haskell x :: Bool = True``` と書く．]

この場合変数 $x$ が $haskell.Bool$ 型であることは自明であるため $x colon.double haskell.Bool$ は省略できるが，可能な限り型を明記しておくことは良い習慣である．また必要に応じて変数に型名を注釈することがある．例えば $x = haskell.True colon.double haskell.Bool$ のように書く場合がある．#footnote[Haskellでは ```haskell x = True :: Bool``` と書く．]
