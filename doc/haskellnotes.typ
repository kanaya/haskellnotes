...
他に整数を表す#keyword[整数型]がある．整数型には2種類あって，その一つは $haskell.Int$ である．この $haskell.Int$ はCの `int` と似た「計算機にとって都合の良い整数」である．計算機にとって都合の良い整数とは，例えば64ビット計算機の場合 $-2^(63)$ から $2^(63)-1$ の間の整数という意味である．#footnote[Haskellでは $haskell.Int$ を ```haskell Int``` で表す．]
