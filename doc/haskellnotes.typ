...
\begin{equation}
  \hxFunc{f}
  \hIsTypeOfhaskell.Int
  \hFunctionArrowhaskell.Int
\end{equation}
という型を持つ．上式は
\begin{equation}
  \hxFunc{f}
  \hIsTypeOf\underbrace{haskell.Int}_{\hxVar{x}}
  \hFunctionArrow\underbrace{haskell.Int}_{\hxFunc{f}\hxVar{x}}
\end{equation}
のようにイメージすると良い．これは関数 $\hxFunc{f}$ が集合 $haskell.Int$ から集合 $haskell.Int$ への#keyword[写像}であると読む．#footnote[Haskellでは \code{f :: Int -> Int} と書く．}

\section{カリー化}

Haskellでは，どのような関数であれ引数は1個しかとらない．引数が2個あるように見える関数として，例えば $\hxFunc{g}\hxVar{x}\hxVar{y}$ があったとしよう．ここに $\hxFunc{g}$ は関数，$\hxVar{x},\hxVar{y}$ は変数である．関数適用は左結合であるから，これは　$\left(\hxFunc{g}\hxVar{x}\right)\hxVar{y}$ である．ここに $\left(\hxFunc{g}\hxVar{x}\right)$ は引数 $\hxVar{y}$ をとる関数であると見ることができる．つまり，関数 $\hxFunc{f}$ とは引数 $\hxVar{x}$ をとり「引数 $\hxVar{y}$ をとって値を返す関数 $\left(\hxFunc{g}\hxVar{x}\right)$ を返す」関数であると言える．
...
\begin{equation}
  (\hIfEq)
  \hIsTypeOfhaskell.Int\hFunctionArrowhaskell.Int\hFunctionArrow\hBool
\end{equation}
であり，浮動小数点型の等値演算子は
\begin{equation}
  (\hIfEq)
  \hIsTypeOfhaskell.Float\hFunctionArrowhaskell.Float\hFunctionArrow\hBool
\end{equation}
である．
...
実は式\eqref{eq:equiv}は不完全なものである．このままでは型 $\hTypeName{a}$に何の制約もないため，等値演算の定義されていない型が来るかもしれないからである．そこで，型自身が所属する，より大きな型があるとしよう．そのような型を我々は#keyword[型クラス}と呼ぶ．例えば型 $\hBool,haskell.Int,haskell.Integer,haskell.Float,haskell.Double$ は全て等値演算が定義できるので，型クラス $\hEq$ に属すとする．この関係を我々は
\begin{equation}
  \hEq
  \hHasElementsOf\hBool,haskell.Int,haskell.Integer,haskell.Float,haskell.Double
\end{equation}
...
\end{equation}
と書くことにする．#footnote[Haskellでは \code{(==) :: Eq a => a -> a -> Bool} と書く．記号 $\hHasElementsOf$ は省略する．}

型 $\hTypeName{a}$ の変数同士の間で大小関係が定義されている場合，かつその型が型クラス $\hEq$ に属する場合，その型は型クラス $\hOrd$ にも属する．型クラス $\hOrd$ に属する型は比較演算子 $<,\le,\ge,>$ を提供する．例えば型 $haskell.Int$ は型クラス $\hOrd$ に属すが，型 $\hBool$ は型クラス $\hOrd$ に属さない．
...
型 $\hTypeName{a}$ が型クラス $\hReal$ 及び型クラス$\hEnum$ に属しているとき，かつそのときに限り，型 $\hTypeName{a}$ は型クラス $haskell.Integral$ にも属する．
...
\begin{equation}
  \hFromIntegral
  \hIsTypeOf{}haskell.Integral\hHasElementsOf\hTypeName{a}
  \hAndThen\hTypeName{a}\hFunctionArrow\hTypeName{b}
\end{equation}
という型を持ち，$haskell.Integral$ 型クラスの型の変数を，任意の型へ変換する．例えば
\begin{align}
  {}
  &\left\{
  \begin{aligned}
  ...
  \right.\\
    ...
    \begin{aligned}
      \hxVar{y}
      &\hIsTypeOfhaskell.Double\\
      \hxVar{y}
      &=\hFromIntegral\hxVar{x}
    \end{aligned}
  \right.
\end{align}
とすることで，$haskell.Int$ 型の変数 $\hxVar{x}$ の値を $haskell.Double$ 型の変数 $\hxVar{y}$ へ代入することができる．#footnote[Haskellでは
\begin{verbatim}
  x :: Int
  x = 1
  y :: Double
...
表\ref{tab:monoids}に型と対応するモノイドの単位元，演算子の一覧を示す．組み合わせ $(haskell.Int,+,\hxConstant{0})$ はモノイドである．同様に $(haskell.Int,*,\hxConstant{1})$, $(haskell.Float,+,\hxConstant{0})$, $(haskell.Float,*,\hxConstant{1})$, $(\hBool,\hLogicalOr,\hFalse)$, $(\hBool,\hLogicalAnd,\hTrue)$ もモノイドである．
...
整数型 $(haskell.Int)$ のリストは $\hListConstruct{haskell.Int}$ と書き，整数のリスト型と呼ぶ．一般に $\hTypeName{a}$ 型のリストを $\hListConstruct{\hTypeName{a}}$ と書く．仮の型である $\hTypeName{a}$ の事を #keyword[型パラメタ}と呼ぶ．
...
\begin{equation}
  ...
  =\hList\,\hTypeName{a}
\end{equation}
とする．この等式の両辺は変数ではなく型名であることに注意しよう．型コンストラクタの概念はPythonには無い（必要無い）が，静的型付け言語である\cxx の「クラステンプレート」が相当する．#footnote[Haskellでは表記上コンテナ型 $\hListConstruct{\hTypeName{a}}$ と型コンストラクタ式$\hList\,\hTypeName{a}$ を区別せず両者とも \code{[a]} と書くが，右辺は \code{[] a} とも書ける．}
...
\begin{equation}
  \hTake
  \hIsTypeOfhaskell.Int\hFunctionArrow\hListConstruct{\hTypeName{a}}\hFunctionArrow\hListConstruct{\hTypeName{a}}
\end{equation}
である．
...
この変数 $\hMaybeVar{w}$ はもはや整数 $(haskell.Int)$ 型とは言えない．そこでこの $\hMaybeVar{z}$ の型を $\hMaybeConstruct{haskell.Int}$ と表して「Maybe整数（おそらく整数）」型と呼ぶことにしよう．型 $\hTypeName{a}$ から型 $\hMaybeConstruct{\hTypeName{a}}$ を生成するこには#keyword[型コンストラクタ} $\hMaybe$ を用いて
\begin{equation}
  \hMaybeConstruct{\hTypeName{a}}
  =\hMaybe\,\hTypeName{a}
\end{equation}
とする．#footnote[Haskellでは表記上コンテナ型 $\hMaybeConstruct{\hTypeName{a}}$ と型コンストラクタ式 $\hMaybe\,\hTypeName{a}$ を区別せず，両者とも \code{Maybe a} と書く．}
...
\end{equation}
のように作られる．#footnote[Haskellでは $\hEitherConstruct{a}{b}$ も $\hEither\,\hTypeName{a}\,\hTypeName{b} $ も区別せずに\code{Either a b} と書く．}
...
\end{equation}
であると考えてみる．型 $\hTypeName{a}$ から型コンストラクタ $\mFuncTypeConstructor$ によって型 $(\mProjEXP{\mR}{\hTypeName{a}})$ が作られると考えるのだ．#footnote[Haskellでは $\mFuncTypeConstructor$ を \code{((->)r)} と書く．}
...
\end{equation}
型クラス $\hOrd$ に属する型は，等号，不等号，小なりイコールと，それらから派生させることのできる大なり $(>)$，大なりイコール $(\ge)$，小なり $(<)$，および最大値をとる関数 $\hTypeName{a}x$ と最小値をとる関数 $\hIsTypeOf$を持つ．#footnote[Haskellでは
\begin{verbatim}
...
\end{equation}
のように，やはり $\mDataType$ を使って定義される．念のため型パラメタを取る場合は $\mDataTypePolymorphic$ と区別しておこう．#footnote[Haskellは $\mDataType$ と $\mDataTypePolymorphic$ を区別せず，$\mDataTypePolymorphic\;\hMaybe\hTypeName{a}=\hJustWith{\hTypeName{a} }\mValueOr\hNothing$ を \code{data Maybe a = Just a | Nothing} と書く．}
...
\right.
\end{multline}
この $\hEq\hHasElementsOf\hTypeName{a} \hAndThen$ の部分が「以下 $\hTypeName{a}$ 型は $\hEq$ 型クラスに属すものとして」という意味になる．#footnote[Haskellでは
\begin{verbatim}
...
$\hTypeName{a}thfrak{f}$ を使い分けてきたが，Haskellプログラム中では全て
\code{f} と書く．
...
\end{equation}
の型パラメタ $\hTypeName{a}$ に $haskell.Float$ 型または $haskell.Double$ 型を
