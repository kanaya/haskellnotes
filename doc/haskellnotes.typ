...
Haskellにおけるモナド（Monad）は、数学の圏論（Category Theory）において定義される「モナド」とほぼ同じ概念である。数学的には、モナドはある圏 $cal(C)$ 上の関手 $frak(T): cal(C)~~>cal(C)$ と、2つの自然変換（単位 $eta$ と乗法 $mu$）から構成される三つ組 $(T, eta, mu)$ であり、以下の条件（モナド則）を満たす：
