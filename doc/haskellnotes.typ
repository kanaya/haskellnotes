...
その答えは，一見遠回りに見えるが，一度抽象数学の高みに登ることである．@bartosz-milewski @bungen-kato
...
と書き直そう．ここに演算子 $compose$ は#keyword[@function-composition-operator]だ．
...
square.stroked.dotted arrow.r.dotted 1+x) $<eq-pattern-matching>
...
であり，関手マップ演算子 $(ast.op.o)$ はアプリカティブマップ演算子 $(ast.square)$ から導出できるからである．ここに $chevron.l f chevron.r$ は文脈に応じて $[f]$ であったり $haskell.Just(f)$ であったりする．記号 $chevron.l square.filled chevron.r$ を#keyword[@pure-operator]と呼ぶ．#footnote[Haskellでは ```haskell pure f``` と書く．]
...
ここに $chevron.l square.filled chevron.r$ はピュア演算子である．また $nothing.rev$ は抽象化された#keyword[@empty]で，型に応じて空リスト $(haskell.emptylist)$ または $haskell.Nothing$ などになる．
...
= プログラミングと代数構造
<ch-programming-and-algebraic-structures>
...
関数は関手である．関手とはマップ演算子 $(ast.op.o)$ を持つ型クラスのことであった．関数が関手であるならば，関数もマップ演算子を持っていないといけない．そこで，関数のマップ演算子とは何かを考えてみる．
...
関数のマップ演算子 $(ast.op.o_(((->)haskell.r)))$ と関数の合成演算子 $(compose)$ は同じ型を持つ．

#tk なぜなら

$ (ast.op.o) &colon.double haskell.Functor supset haskell.f arrow.r.stroked (haskell.a -> haskell.b) -> haskell.fa -> haskell.fb \
  (ast.op.o_(((->)haskell.r))) &colon.double (haskell.a -> haskell.b) -> haskell.typeconstructor1(((->)haskell.r), haskell.a) -> haskell.typeconstructor1(((->)haskell.r), haskell.b) \
...
幸い，我々は関数のマップ演算子の実装に関しては，型さえ守っていれば（そして @ch-functor-and-functor-laws で述べる関手則さえ守っていれば）自由に選べる．そこで
#par-equation($ ast.op.o_(((->)haskell.r)) = compose $)
...
#par-equation($ chevron.l x chevron.r_(((->)haskell.r)) = backslash square.stroked.dotted |-> x $)
...
とする．これは，関数版のピュア演算子の定義と，一般マップ演算子と一般アプリカティブマップ演算子の関係 $f ast.op.o x_* = chevron.l f chevron.r ast.square x_*$ から導かれる．すなわち
#par-equation($ chevron.l g chevron.r ast.square f &= backslash x |-> chevron.l g chevron.r x (f x) \
  &= backslash x |-> (backslash square.stroked.dotted |-> g) x (f x) \
  &= backslash x |-> g (f x) \
  &= g compose f $)
であるからである．
...
#figure(
  ...
  table(
    ...
    [Maybe map], $z_? = f ast.op.o_? x_?$, [```haskell zm = f <$> xm ```],
    [Functor map], $z_* = f ast.op.o x_*$, [```haskell zm = f <$> xm ```],
    [Applicative map], $z_* = f_* ast.square x_*$, [```haskell zm = fm <*> xm ```],
    [Applicative map with 2 arguments], $z_* = g_* ast.square x_* ast.square y_*$, [```haskell zm = gm <*> xm <*> ym ```],
    [Applicative map with 2 arguments], $ z_* &= g ast.op.o x_* ast.square y_* \ &= [| g x_* y_* |]$, [```haskell zm = gm <$> xm <*> ym ```],
...
$ haskell.kwinstance & haskell.Eq supset "TrafficLight" haskell.kwwhere\
  ...
  &square.stroked.dotted equiv square.stroked.dotted = haskell.False $
...
- 単位則: $mu bullet frak(T) eta = mu bullet eta frak(T) = id_frak(T)$ #tk
- 結合則: $mu bullet frak(T) mu = mu bullet mu frak(T)$ #tk

Haskellでは、型コンストラクタ $m$ が $cal(C)$ 上の関手 $frak(T)$ に対応し `return` や `pure` が $eta$、`join` やバインド演算子 (`>>=`) が $mu$ に対応する。つまりHaskellのモナドは、圏論における自己関手と2つの自然変換からなるモナド構造のプログラミング言語への実装例である。

より詳しくは：
- 圏論的モナドは「自己関手と2つの自然変換（$eta$, $mu$）が単位則・結合則を満たす構造」。
- Haskellのモナドは圏論的モナドの公理を関数型プログラミングに持ち込んだもので、型と操作（関数・return・bind）の組としてモナド則を満たす。
