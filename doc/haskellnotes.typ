...
\begin{equation}
  (\hAppend)
  \hIsTypeOf{}\hListConstruct{haskell.a}\hFunctionArrow\hListConstruct{haskell.a}\hFunctionArrow\hListConstruct{haskell.a}
\end{equation}
である．#footnote[Haskellでは $\hListVar{x}\hAppend\hListVar{y}$ を \code{xs++ys} と書く．}

\separator

リストは空でもよい．#keyword[空リスト}は${\hEmptyList}$で表す．#footnote[Haskellでは空リストを \code{[]} で表す．}
...
\begin{equation}
\hNull\hIsTypeOf{}\hListConstruct{haskell.a}\hFunctionArrowhaskell.Bool
\end{equation}
である．
