...
#showybox(title: "式の評価順序")[Haskellは参照透過性を持つ言語なので，式がいつ評価されるかを考える必要はほぼない．一方で参照透過性を持たない言語ででは，式の評価順序をいつも気にしておく必要がある．例えばC言語は関数引数の評価順序を定めていないので，次のコードは画面に `1 2` を出力する場合もあるし `2 1` を出力する場合もある．
#sourcecode[```c
int i = 0;
printf("%d %d", ++i, ++i);
```]
...
Haskellのリスト値コンストラクタは $[x]$ または $x:haskell.emptylist$ で，めったに使われないものの $[]_x$ という書き方も可能である．]
