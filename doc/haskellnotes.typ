...
#figure(
  ...
  table(
    ...
    [有名な値コンストラクタ1], [数学記号], $emptyset, nothing.bar$,
...
型 $haskell.a$ のリストの変数は $x_"s" colon.double [haskell.a]$ という型注釈を持つ．これは $x_"s" colon.double haskell.ListType(haskell.a)$ のシンタックスシュガーである．#footnote[Haskellでは ```haskell xs :: [] a``` と書く．]

型 $haskell.a$ のMaybeの変数は $x_* colon.double haskell.MaybeType(haskell.a)$ という型注釈を持つ．

普段遣いの関数 $f colon.double haskell.a -> haskell.a$ をリスト変数 $x_"s" colon.double [haskell.a]$ に適用する場合は次のように書く．#footnote[Haskellでは ```haskell zs = f `map` xs``` と書く．]
...
型 $haskell.ListType(haskell.a)$ や型 $haskell.MaybeType(haskell.a)$ は型 $haskell.a$ のところに $haskell.Int$ や $haskell.Double$ を代入すると具体的な型となる抽象的な型であった．今度は $[space.thin]$ や $haskell.Maybe$ のほうも $haskell.f$ と抽象化するのである．この $haskell.f$ は#keyword[型コンストラクタ]と呼ぶ．型コンストラクタには，具体的な引数，例えば $haskell.Int$ や $haskell.Double$ を与えると具体的な型になる．
...
#par-equation($ z_* = f convolve.o x_* ast.square y_* ast.square z_* $)
という風にコンテナ変数 $x_*, y_*, z_*$ に関数 $f$ を適用させることができる．コンテナ変数 $x_*, y_*, z_*$ のいずれかが $nothing.rev$ であれば式全体の値が $nothing.rev$ になる．これは3個の計算を並列に行って，その結果をそれぞれ $x'_*, y'_*, z'_*$ に入れておき，最後に関数 $f$ に投げるという#keyword[計算構造]を具現化したものである．（関数 $f$ はCで言えば ```c main``` 関数に相当するであろう．）
