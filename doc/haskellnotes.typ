...
リストの実装について述べておこう．紙上ではリストは自由に考えられるが，計算機上ではそれほど自由ではないからである．我々はリストをLISPにおけるリストと同じ構造を持つものとする．LISPにおけるリストとは変数 `first` と変数 `rest` からなるペアの集合である．変数 `first` がリストの要素を参照し，変数 `rest` が次のペアを参照する．リストの最後のペアの `rest` は空リストを参照する特別な値を持つ．
