...
我々の条件式とは $f x = haskell.kwif x equiv 0 haskell.kwthen 1 haskell.kwelse (sin x) / x$ のように $haskell.kwif$ 節，$haskell.kwthen$ 節，及び $haskell.kwelse$ 節からなるものであって，$haskell.kwthen$ 節も $haskell.kwelse$ 節も省略できないものとする．$haskell.kwif$ 節の式の値が真 $haskell.True$ であれば $haskell.kwthen$ 節の式が評価され，偽 $haskell.False$ であれば $haskell.kwelse$ 節の式が評価される．我々の条件式はCにおける条件演算子（三項演算子）と等しく見えるが，Haskellの場合は遅延評価が行われ
...
タプルの中身の個数は0個または2個以上でなければならず，上限は処理系によって定められている．2個の変数からなるタプルを特別にペア，3個の変数からなるタプルを特別にトリプルと呼ぶ．中身が0個のタプルすなわち $paren.l.flat paren.r.flat$ は特別に#keyword[ユニット]と呼ぶ．#footnote[GHC v8.2.1 は最大62個の変数からなるタプルまで生成できる．]
...
今後，リストを指す変数は，リストであることを忘れないように変数名にsをつけて $x_"s" = [0, 1, ..., 9]$ と書くことにする．なお，変数 $x$ とリスト変数 $x_"s"$ は異なる変数であるとする．#footnote[Haskellでは `s` を変数名にくっつけて `xs = [0, 1..9]}
` のように書く習慣がある．]
...
のように書こう．ここに右辺のリストから一つずつ要素を取り出して左辺に代入する演算子 $in$ を用いた．#footnote[Haskellでは $x_"s" = [x times 2 | x in [0, 1, ..., 9]]$ を `xs = [x * 2 | x <- [0, 1..9]]` と書く．]
...
$[x]$ のように $haskell.a$ 型の変数 $x$ を入れた $[haskell.a]$ 型の変数を作る演算子を#keyword[リスト値コンストラクタ]と呼ぶ．$[haskell.a]$ 型の変数のことを#keyword[リスト変数]とも呼ぶ．$haskell.a$ 型の変数 $x$ からリスト値コンストラクタを使ってリスト$x_"s"$ を作ることは $x_"s" = [x]$ と書く．#footnote[Haskellでは `xs = [x]` と書く．]
...
リストは#keyword[結合]できる．例えばリスト $x_"s"$ とリスト $y_"s"$ を結合したリストは $x smash y$ と表現する．リストの結合演算子の型は
$ smash colon.double [haskell.a] -> [haskell.a] -> [haskell.a] $
である．#footnote[Haskellでは $x_"s" smash y_"s"$ を `xs ++ ys` と書く．]
...
\begin{equation}
  \hFold
  \hIsTypeOf(haskell.a\hFunctionArrow\hTypeName{b}\hFunctionArrowhaskell.a)
...
\end{equation}
これは#keyword[右畳み込み}と呼ばれる演算子である．#footnote[Haskellでは $\hFoldRight^{*}_{\hxVar{a}}\hListVar{x}$ を \code{foldr (*) xs a} と書く．引数の順序に注意しよう．}
...
\end{equation}
と定義することにする．#footnote[Haskellでは演算子 $\mJoinList$ の代わりに \code{concat} 関数（または \code{join} 関数）を使う．}
% ***CHECK***
...
\begin{equation}
  \hMap
  \hIsTypeOf{}(haskell.a\hFunctionArrow\hTypeName{b})\hFunctionArrow\hListConstruct{haskell.a}\hFunctionArrow[\hTypeName{b}]
\end{equation}
である．矢印 $\hFunctionArrow$ は右結合なので，これは
\begin{equation}
  \hMap
  \hIsTypeOf{}(haskell.a\hFunctionArrow\hTypeName{b})\hFunctionArrow\left(\hListConstruct{haskell.a}\hFunctionArrow[\hTypeName{b}]\right)
\end{equation}
の意味でもある．念のため上式に注釈を加えると
\begin{equation}
  \hMap
  \hIsTypeOf\underbrace{\left(haskell.a\hFunctionArrow\hTypeName{b}\right)}_{\hxFunc{f}}
...
\begin{align}
  \hxFunc{f}
  &\hIsTypeOfhaskell.a\hFunctionArrow\hTypeName{b}\\
  \hxFunc{f}\hMap
  &\hIsTypeOf{}\hListConstruct{haskell.a}\hFunctionArrow[\hTypeName{b}]
\end{align}
...
\item マップ演算子$\hMap$ の型は$\mProjEXP{(\mProjEXP{haskell.a }{\hTypeName{b}})}{\mProjEXP{haskell.a }{\hTypeName{b} }}$である．
...
ここで変数 $\hMaybeVar{w}$ が取り得る値は正しく計算された値 $\hxVar{z}$ をラップしたものか，あるいはエラーを表す値 $\hNothing$ である．このように計算結果に「意味付け」をすることを#keyword[文脈}に入れると言う．定数 $\hNothing$ は「ナッシング」と呼ぶ．#footnote[Haskellでは $\hNothing$ を \code{Nothing} と書く．}

この変数 $\hMaybeVar{w}$ はもはや整数 $(haskell.Int)$ 型とは言えない．そこでこの $\hMaybeVar{z}$ の型を $\hMaybeConstruct{haskell.Int}$ と表して「Maybe整数（おそらく整数）」型と呼ぶことにしよう．型 $haskell.a$ から型 $\hMaybeConstruct{haskell.a}$ を生成するこには#keyword[型コンストラクタ} $\hMaybe$ を用いて
\begin{equation}
  \hMaybeConstruct{haskell.a}
  =\hMaybe\,haskell.a
\end{equation}
とする．#footnote[Haskellでは表記上コンテナ型 $\hMaybeConstruct{haskell.a}$ と型コンストラクタ式 $\hMaybe\,haskell.a$ を区別せず，両者とも \code{Maybe a} と書く．}

$haskell.a$ 型の変数を $\hMaybeConstruct{haskell.a}$ 型の変数に代入するには，次のMaybe値コンストラクタを用いて
\begin{equation}
  \hMaybeVar{w}
  =\hJustWith{\hxVar{z}}
\end{equation}
とする．#footnote[Haskellでは $\hMaybeVar{w}=\hJustWith{\hxVar{x}}$ を \code{w = Just x} と書く．Maybeを表す疑問符は省略する．}
...
\end{equation}
となるようなMaybeバージョンのマップ演算子 $\hFunctorMap$ を用いる．ここに $\hxFunc{f}\hFunctorMap\hMaybeVar{w}$ の型は，もし $\hxFunc{f}\hIsTypeOf\mProjEXP{haskell.Int}{haskell.Float}$ ならば $\hMaybeConstruct{haskell.Float}$ である．#footnote[Haskellでは $\hxFunc{f}\hFunctorMap\hMaybeVar{w}$ を \code{f<\$>w} と書く．}
...
\begin{equation}
  ...
  =\hxFunc{f}\hFunctorMap\hMaybeVar{w}
\end{equation}
のようにMaybeバージョンのマップ演算子 $\hFunctorMap$ を用いることにする．これはプログラムの安全性のためである．変数が一旦ゼロ除算の可能性に汚染されたら，最後までMaybeに包んでおかねばならない．% #footnote[HaskellではMaybeバージョンのマップ演算子に特別な記号，関数名が与えられていない．その代わり第\ref{ch:functor}章で述べる一般マップ演算子 $\mMap$ に相当する \code{fmap} 関数を用い \code{u = fmap f w} または \code{u = f <\$> w} のように書く．Haskellは型推論を行なうため変数 \code{w} がMaybeであればMaybeバージョンのマップ演算子（関数）が適用され，もし \code{w} がリストであれば通常のマップ関数である \code{map} が適用される．} %%% !!!!

PythonでMaybeの概念を忠実になぞることは難しい．と言うのもPythonは動的型付け言語であるため，型コンストラクタという概念が無いからだ．一方でMaybeの概念を静的型付け言語である\cxx や\java で実現することはできる．そこで\cxx の本物のコードで示しておこう．ただしポインタを使わないでおいたので\cxx プログラマも\java プログラマも参考にできるだろう．
...
Maybeとよく似た型にEitherがある．Maybeが $haskell.a$ 型または $\hNothing$ のいずれかの値をとったように，Eitherは $haskell.a$ 型または $\hTypeName{b}$ 型のいずれかの値を取る．$haskell.a$ 型または $\hTypeName{b}$ 型を取るEither型の変数 $\hEitherVar{e}$ があるとすると，
\begin{equation}
  \hEitherVar{e}
  \hIsTypeOf{}\hEitherConstruct{haskell.a}{\hTypeName{b}}
\end{equation}
と書く．Either型は型 $haskell.a$ および $\hTypeName{b}$ から型コンストラクタを用いて
\begin{equation}
  \hEitherConstruct{haskell.a}{\hTypeName{b}}
  =\hEither\,haskell.a \,\hTypeName{b}
\end{equation}
のように作られる．#footnote[Haskellでは $\hEitherConstruct{a}{b}$ も $\hEither\,haskell.a\,\hTypeName{b} $ も区別せずに\code{Either a b} と書く．}
...
\item 普通の関数 $\hxFunc{f}\hIsTypeOf\mProjEXP{haskell.a }{\hTypeName{b} }$をMaybe値に適用することはできない．関数 $\hxFunc{f}$ をMaybe値に適用するには$\hxFunc{f}\hFunctorMap\hMaybeVar{u}$ のようにMaybeマップ演算子が必要であり，この関数適用の結果は $\hMaybeConstruct{b}$ 型である．
...
\begin{equation}
  (\hFunctorMap)\hIsTypeOf\hFunctor\hHasElementsOf\hTypeConstructor{f}\hAndThen
    (haskell.a\hFunctionArrow\hTypeName{b})\hFunctionArrow\hTypeConstruct{\hTypeConstructor{f}}{haskell.a}\hFunctionArrow\hTypeConstruct{\hTypeConstructor{f}}{\hTypeName{b}}
\end{equation}
...
\begin{equation}
  (\hCompose)\hIsTypeOf{}(haskell.a\hFunctionArrow\hTypeName{b})\hFunctionArrow\hTypeConstruct{(\hFunctionArrow)\hTypeName{r}}{haskell.a}\hFunctionArrow\hTypeConstruct{(\hFunctionArrow)\hTypeName{r}}{\hTypeName{b}}
\end{equation}
...
いま「世界」と呼んだものを，数学者は#keyword[圏}と呼ぶ．圏とは#keyword[対象}と#keyword[射}の組み合わせである．本書では「対象」とは型のことであり，射とは関数だと思えば良い．（厳密にはコンテナに入れられた関数も射に含まれる．）そして，圏から圏へとジャンプさせるものを#keyword[関手}と呼ぶ．この例で言えば値コンストラクタ $\hJustWith{\hxVar{x}}$ とマップ演算子 $\hFunctorMap$ が関手である．値コンストラクタ $\hJustWith{\hxVar{x}}$ は $haskell.a\hFunctionArrow\hMaybeConstruct{haskell.a}$ という型を持ち，マップ演算子 $\hFunctorMap$ は $(haskell.a\hFunctionArrow\hTypeName{b})\hFunctionArrow(\hMaybeConstruct{haskell.a}\hFunctionArrow\hMaybeConstruct{b})$ という型を持つ．#footnote[関手は英語でファンクター(functor)と言うが，\cxx の関数オブジェクト (function object) もかつてはファンクター(functor)と呼ばれていた．\cxx のファンクターとはクロージャの代用品のことで，本書で述べる関手とは異なる概念である．混同しないように注意しよう．}

同じことはリストにも言える．値コンストラクタ $[\hxVar{x}]$ とマップ演算子 $\hMap$ もまた関手である．この場合値コンストラクタは $haskell.a\hFunctionArrow\hListConstruct{haskell.a}$ という型を持ち，マップ演算子も同じく $(haskell.a\hFunctionArrow\hTypeName{b})\hFunctionArrow(\hListConstruct{haskell.a}\hFunctionArrow[\hTypeName{b}])$ という型を持つ．
...
\begin{equation}
  ...
  \hAndThen(haskell.a\hFunctionArrow\hTypeName{b})
...
\begin{equation}
  (\hFunctorMap)
  \hIsTypeOf{}(haskell.a\hFunctionArrow\hTypeName{b})\hFunctionArrow\hMaybeConstruct{haskell.a}\hFunctionArrow\hMaybeConstruct{b}
\end{equation}
である．
...
$\hMaybeConstruct{haskell.a}$&$\hMaybe$&$\hFunctorMap$&$\hJustWith{\hxVar{x}},\hNothing$\\\hline
\end{tabular}
\end{center}
\end{table*}
...
$[haskell.a\hFunctionArrow\hTypeName{b}]\hFunctionArrow\hListConstruct{haskell.a}\hFunctionArrow[\hTypeName{b}]$ である．これは
\begin{equation}
  haskell.appMapList
  \hIsTypeOf\underbrace{[haskell.ahaskell.apsto\hTypeName{b}]}_{[\hxFunc{f}_0,\hxFunc{f}_1\dotsb \hxFunc{f}_n]}
  haskell.apsto\underbrace{\hListConstruct{haskell.a}}_{[\hxVar{x}_0,\hxVar{x}_1\dotsb \hxVar{x}_n]}
  haskell.apsto\underbrace{[\hTypeName{b}]}_{[\hxFunc{f}_0\hxVar{x}_0,\hxFunc{f}_0\hxVar{x}_1\dotsb \hxFunc{f}_0\hxVar{x}_n,\hxFunc{f} \hxVar{x}_0,\hxFunc{f} \hxVar{x}_1\dotsb \hxFunc{f}_n\hxVar{x}_n]}
\end{equation}
と解釈すれば良い．
...
\begin{equation}
  (\mMapFunc)
  \hIsTypeOf{}(haskell.a\hFunctionArrow\hTypeName{b})
  \hFunctionArrow\mFuncType{a}
  \hFunctionArrow\mFuncType{b}
\end{equation}
であり，これはすなわち
...
いま関数 $\hxFunc{f}\hIsTypeOf\mProjEXP{\mR}{haskell.a }$ とは別な関数 $g\hIsTypeOf\mProjEXP{haskell.a }{\hTypeName{b} }$ があったとしよう．関数 $\hxFunc{f}$ と関数 $\hxFunc{g}$ の合成 $\hxFunc{g}\hCompose\hxFunc{f}$ の型は
\begin{equation}
\hxFunc{g}\hCompose\hxFunc{f}\hIsTypeOf\mProjEXP{\mR}{\hTypeName{b} }
\end{equation}
であるから，
\begin{equation}
(\hCompose)\hIsTypeOf{}\mProjEXP{\mProjEXP{(\mProjEXP{haskell.a }{\hTypeName{b} })}{(\mProjEXP{\mR}{haskell.a })}}
...
\begin{equation}
  (haskell.appMapFunc)
  \hIsTypeOf\mFuncType{haskell.a\hFunctionArrow\hTypeName{b}}
...
\begin{equation}
  (haskell.appMapFunc)
  \hIsTypeOf{}(\mR\hFunctionArrowhaskell.a\hFunctionArrow\hTypeName{b})
  \hFunctionArrow(\mR\hFunctionArrowhaskell.a)
  \hFunctionArrow(\mR\hFunctionArrow\hTypeName{b})
\end{equation}
である．
...
\end{equation}
である．ということは，関数 $\hxFunc{g}$ をMaybe変数に適用させるようと思っても，我々が既に知っているマップ演算子 $\hFunctorMap$ やアプリカティブマップ演算子 $haskell.appMapMaybe$ が使えないということである．前者は第1引数に $\mProjEXP{haskell.a }{\hTypeName{b} }$ 型の関数を取るし，後者は第1引数に $\hJustWith{(\mProjEXP{haskell.a }{\hTypeName{b} })}$ 型の関数（Maybe関数）を取るからである．
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
ここに $\hxVar{x}$ は非コンテナ変数で，関数 $\hxFunc{f}$ も「普通の」（コンテナに入っていない）関数である．より厳密に言えば $\hxVar{x}\hIsTypeOfhaskell.a$ かつ $\hxFunc{f}\hIsTypeOfhaskell.a\hFunctionArrow\hTypeName{b}$ である．従って，関数 $\mVarContainer{i}$ の型は $haskell.a\hFunctionArrow\mPureType{b}$ である．条件 $\mLovelyVar$ には任意の値を入れてよい．
...
\mDoNext\beta z}\\
=\beta\hTypeName{b}ind{}(\hxFunc{f}\mMap\alpha xhaskell.appMap\alpha'x')
\end{multline}
...
\end{equation}
であるとする．#footnote[Haskellでは $\alpha x\hTypeName{b}indRight\beta$ を \code{alpha x >>= beta} と書く．}
...
\end{equation}
なる演算子 $\hTypeName{b}indRightIgnore$ を用いて記述される．#footnote[Haskellでは $\alpha_1x\hTypeName{b}indRightIgnore\alpha_2$ を \code{alpha1 x >> alpha2} と書く．}
...
\begin{equation}
  ...
  \hMaybe\,haskell.a
  =\hJustWith{haskell.a}\mValueOr\hNothing
\end{equation}
のように，やはり $\mDataType$ を使って定義される．念のため型パラメタを取る場合は $\mDataTypePolymorphic$ と区別しておこう．#footnote[Haskellは $\mDataType$ と $\mDataTypePolymorphic$ を区別せず，$\mDataTypePolymorphic\;\hMaybehaskell.a=\hJustWith{haskell.a }\mValueOr\hNothing$ を \code{data Maybe a = Just a | Nothing} と書く．}
...
\hNothing\hIfEq\hNothing&=haskell.True\\
\_\hIfEq\_&=haskell.False
\end{aligned}
\right.
\end{multline}
...
\begin{equation}
\mDataTypePolymorphic\;\hEither\,haskell.a \,\hTypeName{b}
=\hLeftWith{haskell.a }
\mValueOr
...
\begin{equation}
  ...
  \begin{aligned}
    {}&\mZipWith\hIsTypeOf{}\mProjEXP{\mProjEXP{\mProjEXP{(\mProjEXP{haskell.a }
        ...
        {\hListConstruct{haskell.a}}}{[\hTypeName{b}]}}{[\mC]}\\
    {}&\left\{\begin{aligned}
...
\begin{equation}
  ...
  \begin{aligned}
    ...
    {\mProjEXP{[\hTypeName{b}]}
      {(\hListConstruct{haskell.a},[\hTypeName{b}])}}\\
...
\begin{equation}
  ...
  \begin{aligned}
    {}&\hUncurry\hIsTypeOf{}
    \mProjEXP{(\mProjEXP{haskell.a }{\mProjEXP{\hTypeName{b} }{\mC }})}
          {\mProjEXP{(haskell.a ,\hTypeName{b} )}{\mC }}\\
...
\begin{equation}
  \mNewTypeDecl\;
  \mTypeConstructor{NonDet}\,haskell.a\,\hTypeName{b}
  =\mValueRecordWith{NonDet}{\mRun\hIsTypeOfhaskell.a\hFunctionArrow[\hTypeName{b}]}
\end{equation}
とする．#footnote[Haskellでは
\begin{verbatim}
...
$\mTypeAssemble{NonDet}{haskell.a\,\hTypeName{b}}$ は型 $haskell.a\hFunctionArrow\hListConstruct{haskell.a}$ とは異な
...
まで型 $haskell.a$ の引数をひとつ取り，型 $\hTypeName{b}$ の戻り値を返す関数の型を
$haskell.a\hFunctionArrow\hTypeName{b}$ または $(\hFunctionArrow)haskell.a\hTypeName{b}$ と書いてきた．これか
ら導入する型は，やはり型 $haskell.a$ の引数を取り，型 $\hTypeName{b}$ の戻り値を返す．
この新しい型を $haskell.a\mCatArrowC\hTypeName{b}$ または $(\mCatArrowC)haskell.a\hTypeName{b}$ と書こ
う．ここに $\mTypeConstructor{c}$ は型コンストラクタであり，
カテゴリ型クラスのインスタンスである．型 $(\mCatArrowC)haskell.a\hTypeName{b}$ の値は
関数ではなく#keyword[射}と呼ぶ．#footnote[Haskellでは
  $(\mCatArrowC)haskell.a\hTypeName{b}$ を \code{c a b} と書く．}
...
\begin{align}
  \mIdCat
  &\hIsTypeOf{}haskell.a\mCatArrowC\hTypeName{b}\\
  (\mCompCat)
  &\hIsTypeOf{}(\hTypeName{b}\mCatArrowC\mC)
  \hFunctionArrow(haskell.a\mCatArrowC\hTypeName{b})
...
\begin{equation}
  haskell.a\mCatArrowC\hTypeName{b}
  =haskell.a\hFunctionArrow\hListConstruct{haskell.a}
\end{equation}
...
は型 $\mTypeAssemble{NonDet}{haskell.a\,\hTypeName{b}}$ に対応していないので，次
...
\begin{multline}
  ...
  \right.
\end{multline}
続けて，$\mTypeAssemble{NonDet}{haskell.a\,\hTypeName{b}}$ 型バージョンの
...
\begin{equation}
  \mNewTypeDecl\;
  \mTypeConstructor{ItCouldBe}\,haskell.a\,\hTypeName{b}
...
\begin{align}
  \mNewTypeDecl\;
  &\mTypeConstructor{NonDet}\,haskell.a\,\hTypeName{b}
  =\mValueWith{NonDet}{haskell.a\hFunctionArrow[\hTypeName{b}]}\\
  \mNewTypeDecl\;
  &\mTypeConstructor{ItCouldBe}\,haskell.a\,\hTypeName{b}
...
\begin{equation}
  \mNewTypeDecl\;
  \mTypeConstructor{Kleisli}\,\mTypeConstructor{m}\,haskell.a\,\hTypeName{b}
  =\mValueWith{Kleisli}{haskell.a\hFunctionArrow\mTypeAssemble{m}{\hTypeName{b}}}
\end{equation}

\begin{equation}
  \mTypeSynonymDecl\;
  \mTypeConstructor{ItCouldBe}\,haskell.a\,\hTypeName{b}
  =\mTypeConstructor{Kleisli}\,haskell.a\,\hTypeName{b}
\end{equation}
...
$\hTypeName{b}haskell.arrowArrow{\mTypeConstructor{a}}\mC$ という型を持つとす
る．

型 $\hTypeName{b}\hFunctionArrow\mC$ は $(\hFunctionArrow)\hTypeName{b}\mC$ の中置記法バージョンで
あった．我々の $\hTypeName{b}haskell.arrowArrow{\mTypeConstructor{a}}\mC$ も
$(haskell.arrowArrow{\mTypeConstructor{a}})\hTypeName{b}\mC$ の中置記法バージョ
...
\begin{multline}
  ...
  \begin{aligned}
    ...
    \hFunctionArrow(\hTypeName{b}haskell.arrowArrow{\mTypeConstructor{a}}\mC)\\
    \mFirst
    &\hIsTypeOf{}(\hTypeName{b}haskell.arrowArrow{\mTypeConstructor{a}}\mC)
...
\begin{equation}
  \mNewTypeDecl\;
  \mTypeConstructor{SimpleFunc}\,haskell.a\,\hTypeName{b}
  =\mValueRecordWith{SimlpeFunc}{\mFuncall\hIsTypeOfhaskell.a\hFunctionArrow\hTypeName{b}}
\end{equation}
型 $\mTypeConstructor{SimpleFunc}\,haskell.a\,\hTypeName{b}$ は $haskell.a\hFunctionArrow\hTypeName{b}$ 型
...
\begin{align}
  ...
  \begin{aligned}
    ...
    \hAndThen\hTypeName{b}haskell.arrowArrow{\mTypeConstructor{a}}(\hTypeName{b},\hTypeName{b})\\
    &\mSplit'
  ...
  \begin{aligned}
    ...
    \hFunctionArrow\left(\hPairWith{\hTypeName{b}}{\mC}haskell.arrowArrow{\mTypeConstructor{a}}\mD\right)\\
    &\mUnsplit'
...
これまでの関数の型は $haskell.a\hFunctionArrow\hTypeName{b}$ であった．
これから新しい「関数風」の型 $haskell.a\mCatArrowC\hTypeName{b}$ を考える．
...
\begin{equation}
  \mNewTypeDecl\;
  \mCircuitType\,haskell.a\,\hTypeName{b}
  =\mCircuitWith{\mUncircuit\hIsTypeOfhaskell.a\hFunctionArrow(\mCircuitType\,haskell.a\,\hTypeName{b},\hTypeName{b})}
\end{equation}
...
\begin{align}
  \mRunCircuit&\hIsTypeOf\mCircuitType\,haskell.a\,\hTypeName{b}\hFunctionArrow\hListConstruct{haskell.a}\hFunctionArrow[\hTypeName{b}]\\
...
\begin{equation}
  \mNewTypeDecl\;
  \mKleisliType\,\mTypeConstructor{m}\,haskell.a\,\hTypeName{b}
...
\begin{equation}
\mMain=(\dotsb\mWhere{}\hListVar{a}=haskell.action{getArgs})\hTypeName{b}indRightIgnore\mPureWith{0}
\end{equation}
