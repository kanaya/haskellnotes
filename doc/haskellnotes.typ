...
#figure(
  ...
  table(
    ...
    [有名な値コンストラクタ2], [特別な括弧で包む], $[x], paren.l.stroked u, v paren.r.stroked, chevron.l y chevron.r$,
...
まずHomebrewというパッケージマネージャをmacOSにインストールする．それには，ターミナルを立ち上げてコマンドシェルから次のようにする．
...
このPython関数 ```python create_max(x)``` は我々の $max x$ と同じ用に，1引数を取る関数として振る舞う．そのため ```python create_max(10)(20)``` は ```python 20``` を返す．このような仕組みは#keyword[@closure]または関数閉包と呼ばれる．
...
局所変数はラムダ式を用いたシンタックスシュガーである．@eq-let-in は次の式と等価である．
...
ここに $forall$ は#keyword[@universal-quantifier]という記号で，ラムダ式におけるラムダ記号 $(backslash)$ に相当する．Haskellは小文字で始まる型名を型パラメタとみなすため，全称量化子を省略しても型を正しく推論できるのである．
...
型 $haskell.a$ から型 $[haskell.a]$ を生成する演算子を#keyword[@list-type-constructor]と呼んで $[space.thin]$ と書く．型 $[haskell.a]$ は $haskell.ListType(haskell.a)$ のシンタックスシュガーである．#footnote[Haskellでは $haskell.ListType(haskell.a)$ を ```haskell [] a``` と書く．これは ```haskell [a]``` と同じことである．]
...
この新しい演算子 $haskell.fold$ は#keyword[@fold-operator]と呼ばれる．変数 $a$ を#keyword[@accumulator]と呼ぶ．アキュムレータは右側の引数が空であった場合のデフォルト値と考えても良い．#footnote[Haskellでは $haskell.fold_a^+ x_"s"$ を ```haskell foldl (+)) a xs``` と書く．]
...
であるから，演算子 $haskell.fold_haskell.emptylist^smash$ はリストを平坦化する#keyword[@flatten-operator]である．平坦化演算子はconcat演算子とも呼ばれることもあるが，基本的な演算子であるため特別な記号をつけておこう．我々は
...
演算子 $*_"M"$ は#keyword[@action]版のマップ演算子である．アクションについては @ch-monad で扱う．
...
Haskellにはシンタックスシュガーとして逐次処理記法が導入されている．それが#keyword[@do-notation]である．標準入力から1行を読み取って変数 $s$ に代入し，それを標準出力に書き出すHaskellプログラムを我々はこれまで
