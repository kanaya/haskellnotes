...
関数型プログラミングの特徴を一言で言えば，プログラム中の#keyword[@destructive-assignment]を禁止することである．変数 $x$ に $1$ という数値が一度代入されたら，変数 $x$ の値をプログラム中に書き換える，すなわち破壊的代入をすることはできない．この結果，変数の値はプログラムのどこでも，どの時点で読み出しても同じであることが保証される．これを変数の#keyword[@referential-transparency]と呼ぶ．#footnote[変数の値が常に同じなことに加えて，関数に同じ引数を与えた場合に常に同じ結果が得られることを参照透過性と呼ぶ．]
...
変数の値がいつでも変化しないことを#keyword[@referential-transparency]と呼ぶ．#footnote[関数に同じ引数を与えた場合，同じ値がいつも返ることも参照透過性の条件である．ただし，関数が内部状態を持つためには，どこかで破壊的代入が必要である．]  // 本当???
