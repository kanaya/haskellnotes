...
$ f_1 &= "tr"_([A...Z]->[a...z])\
  f_2 &= "tr"_(overline([a...z, square])->haskell.emptylist)\
...
なお英語のファンクションのほうにも日本語の「関数」の意味と「機能」の意味の両方がある．C言語の関数（function）はどちらかと言えば「機能」の意味である．
...
なおLispやSchemeはラムダ式がそのプログラミングの本質であるにも関わらず ```scheme lambda``` という長いキーワードを用いている．例えば $backslash x y |-> x + y$ は ```sceme (lambda (x y) (+ x y))``` と書かれる．もっともLispプログラマ，Schemeプログラマは ```scheme lamda``` が1文字に見えている可能性はある．]
