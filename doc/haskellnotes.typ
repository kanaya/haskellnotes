...
- Haskellでは変数への値の代入を「束縛」と呼ぶ．
- 変数やリテラルには型がある．整数型は `Int`，浮動小数点型は `Double`．
- 型注釈は `x :: Int` のように書くことができる．
- 変数の値は一度束縛すると変化しない（参照透過性）．
- Pythonなどで見られる「破壊的代入」はせず，値は不変．
- 整数 $x$ に $1$ を足す関数 $f$ は $f x = x + 1$ のように定義し，カリー風記法を用いる．
- Haskellでは関数適用を `f(x)` ではなく `f x` のように括弧なしで書く．
- 関数名や変数名は1文字でイタリック体とし，$f,g,h,x,y,z$ などの文字を主に利用する．
- 関数 $f$ に引数 $x$ を与えることを「適用」と呼ぶ．
- 関数は左から右へ適用し，積とは区別する必要がある．
