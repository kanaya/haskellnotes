...
Haskellにはよく使う型が予め用意されている．例えば#keyword[論理型]は論理値すなわち真 ($haskell.True$) または偽 ($haskell.False$) という値をとる変数の型である．ある変数 $x$ が論理型であることを，Haskellでは
#par-equation($ x colon.double haskell.Bool $)
と書く．数学者なら同じことを $x in BB$ と書くところであるが，ここはHaskellの流儀に従おう．また型定義と値定義はよく一緒に行われるので，今後 
#par-equation($ x colon.double haskell.Bool = haskell.True $)
のようにまとめて書くことにしよう．#footnote[Haskellでは
```haskell
  x :: Bool
  x = True
```
...
この場合変数 $x$ が $haskell.Bool$ 型であることは自明であるため $x colon.double haskell.Bool$ は省略できるが，可能な限り型を明記しておくことは良い習慣である．また必要に応じて変数に型名を注釈することがある．例えば $x = haskell.True colon.double haskell.Bool$ のように書く場合がある．#footnote[Haskellでは $x = haskell.True colon.double haskell.Bool$ を ```haskell x = True :: Bool``` と書く．]

他に整数を表す#keyword[整数型]がある．整数型には2種類あって，その一つは $haskell.Int$ である．この $haskell.Int$ はCの ```c int``` と似た「計算機にとって都合の良い整数」である．計算機にとって都合の良い整数とは，例えば64ビット計算機の場合 $-2^(63)$ から $2^(63)-1$ の間の整数という意味である．#footnote[Haskellでは $haskell.Int$ を ```haskell Int``` で表す．]

整数型には $haskell.Integer$ もある．この $haskell.Integer$ は計算機にとっては非常識なぐらい大きな，あるいは小さな値を表すことができる．#footnote[Haskellでは $haskell.Integer$ を ```haskell Integer``` で表す．]
...
のようにイメージすると良い．これは関数 $f$ が集合 $haskell.Int$ から集合 $haskell.Int$ への#keyword[写像]であると読む．#footnote[Haskellでは ```haskell f :: Int -> Int``` と書く．]
...
#par-equation($ haskell.fact x = haskell.kwcase x haskell.kwof cases(0 --> 1,
...
ここでリストの実装について述べておこう．紙上ではリストは自由に考えられるが，計算機上ではそれほど自由ではないからである．我々はリストをLISPにおけるリストと同じ構造を持つものとする．LISPにおけるリストとは変数 `first` と変数 `rest` からなるペアの集合である．変数 `first` がリストの要素を参照し，変数 `rest` が次のペアを参照する．リストの最後のペアの `rest` は空リストを参照する特別な値を持つ．
...
#par-equation($ f * emptyset &= emptyset \
  f * (x : x_"s") &= (f x) : (f * x_"s") $)
...
#par-equation($ haskell.fact x = haskell.kwcase x haskell.kwof cases(0 --> 1, rect.stroked.h --> x times haskell.fact(x - 1)) $)
のような形をしていた．末尾の関数をよりはっきりさせるために演算子 $(*)$ を前置にして
#par-equation($ haskell.fact x = haskell.kwcase x haskell.kwof cases(0 --> 1, rect.stroked.h --> (times) x space haskell.fact(x - 1)) $)
...
#par-equation($ haskell.fact' a x = haskell.kwcase x haskell.kwof cases(0 --> a, rect.stroked.h --> haskell.fact' (a times x) space (x - 1)) $)
...
$ f convolve.o haskell.Just(x) &= haskell.Just(f x) \
  f convolve.o haskell.Nothing &= haskell.Nothing $
...
$ haskell.id convolve.o &= haskell.id \
...
$ z_* = g convolve.o x_* haskell.amap y_* $
...
いま「世界」と呼んだものを，数学者は#keyword[圏]と呼ぶ．圏とは#keyword[対象]と#keyword[射]の組み合わせである．本書では「対象」とは型のことであり，射とは関数だと思えば良い．（厳密にはコンテナに入れられた関数も射に含まれる．）そして，圏から圏へとジャンプさせるものを#keyword[関手]と呼ぶ．この例で言えば値コンストラクタ $haskell.Just(x)$ とマップ演算子 $convolve.o$ が関手である．値コンストラクタ $haskell.Just(x)$ は $haskell.a -> haskell.MaybeType(haskell.a)$ という型を持ち，マップ演算子 $convolve.o$ は $(haskell.a -> haskell.b) -> (haskell.MaybeType(haskell.a) -> haskell.MaybeType(haskell.b))$ という型を持つ．#footnote[関手は英語でファンクター(functor)と言うが，C++の関数オブジェクト (function object) もかつてはファンクター(functor)と呼ばれていた．C++のファンクターとはクロージャの代用品のことで，本書で述べる関手とは異なる概念である．混同しないように注意しよう．]
...
Haskellではマップ演算子が定義された型を関手（型）と呼ぶ．具体的には，マップ演算子が定義された全ての型は $haskell.Functor$ 型クラスのインスタンスであるとする．つまり，$haskell.Functor$ 型クラスには一般化されたマップ演算子が定義されており，そのインスタンスであるリストやMaybeは独自のマップ演算子を定義しなければならないということである．

一般化されたマップ演算子を $bullet.o$ で表そう．この $bullet.o$ 演算子は
$ bullet.o colon.double haskell.Functor supset haskell.f 
  ==> (haskell.a -> haskell.b) -> haskell.f_haskell.a -> haskell.f_haskell.b $
...
$haskell.f = haskell.Maybe$ であれば
$ convolve.o colon.double (haskell.a -> haskell.b) -> haskell.MaybeType(haskell.a) -> haskell.MaybeType(haskell.b) $
である．
...
$ haskell.id convolve.o x_* &= haskell.id x_* \
