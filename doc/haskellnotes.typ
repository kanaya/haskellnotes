...
古典的には `goto` 命令とラベルの組み合わせが大域ジャンプに使われる．プログラム中の `goto` 命令には大した害が無いものの，ラベルはプログラムの可読性を著しく低下させる．ラベルだけでは，どこから飛んできたのか判断がつかないからである．

そもそも大域ジャンプが必要なのはエラー処理などの「例外的な事象」に限ると割り切って，専用の構文を導入したのがプログラミングにおける例外機構である．例外機構によってエラー処理の見通しは良くなるが，例外機構は参照透過性を破壊するため，関数型プログラミング言語では扱いにくい．そのためにMaybeを導入したのがHaskellであり，オプショナルなどMaybeの劣化コピーが多くのオブジェクト指向プログラミング言語で採用されている．

なお再帰では分かりづらいループに対して `goto` を使うことが手続き型プログラミングでは多く見受けられるが，Schemeのような言語では `goto` を使う代わりに「継続」を使うことによって見通しを良くしている．]
