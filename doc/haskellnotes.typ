...
#sourcecode[```python
x_"s" = [x + y for x in range(0, 10) for y in range(0, 6) if x + y > 3]
```]
...
$ f &= backslash x |-> 100 + x \
  x_"s" &= [1, 2, ..., 5] \
  haskell.y_"s" &= f * x_"s" $
とすると $haskell.y_"s"$ の値は $101, 102, 103, 104, 105]$ となる．
...
$ f ast.op.o w_"?" = haskell.kwcase w_"?" haskell.kwof
  cases(haskell.Just(x) --> haskell.Just(f x),
    dash.wave.double --> haskell.Nothing) $
