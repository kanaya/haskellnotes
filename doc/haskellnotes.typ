...
rect.stroked.h arrow.r.dotted x + 1) $

ここに $rect.stroked.h$ は任意の値の意味である．パタンマッチは上から順番にマッチングしていくため，この場合は $0$ 以外を意味する．#footnote[Haskellでは ```haskell f' x = case x of { 0 -> 100; _ -> x + 1 }``` と書く．]
...
文字列型のリテラルは次のように書く．#footnote[Haskell では ```haskell ts = "Hello, World!"``` と書く．]
...
他に $haskell.a$ 世界から $[haskell.a]$ 世界や $haskell.MaybeA$ 世界へとジャンプさせるものがあるだろうか．よく考えてみると，マップ演算子もそうである．なぜなら $u_"s" = [x], v_"s" = [y]$ なのだから $haskell.a$ 世界の関数 $f$ と $[haskell.a]$ 世界の関数 $g$ は無関係ではなく
...
であり，関手マップ演算子 $(convolve.o)$ はアプリカティブマップ演算子 $(ast.square)$ から導出できるからである．ここに $chevron.l f chevron.r$ は文脈に応じて $[f]$ であったり $haskell.Just(f)$ であったりする．記号 $chevron.l square.stroked.dotted chevron.r$ を#keyword[ピュア演算子]と呼ぶ．#footnote[Haskellでは ```haskell pure f``` と書く．]
...
$ z_* = chevron.l g chevron.r ast.square x_* ast.square y_* $<applicative-style>
...
アプリカティブマップ演算子を用いると#keyword[アプリカティブスタイル]という記法が使える．関数 $f, g, h$ が引数を文脈に入れる関数であるとしよう．すると，アプリカティブスタイルでは次のようにコンテナ変数 $u_*, v_*, w_*$ に関数 $i$ を適用させることができる．

$  u_* &= f x \
  v_* &= g y \
  w_* &= h convolve.o u_* ast.square v_* $

コンテナ変数 $u_*, v_*, w_*$ のいずれかが $nothing.rev$ であれば式全体の値が $nothing.rev$ になる．これは3個の計算を並列に行って，その結果をそれぞれ $u_*, v_*, w_*$ に入れておくことを意味する．このような書き方をアプリカティブスタイルと呼ぶ．
