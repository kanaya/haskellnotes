...
Haskellのすべての変数，関数には#keyword[@type]がある．代表的な型には整数型，浮動小数点型，ブール型，文字型がある．整数型を $haskell.Int$ で，浮動小数点型を $haskell.Double$ で表すことはすでに述べたとおりである．
...
#keyword[@boolean]は論理値 $haskell.True$ または $haskell.False$ のいずれかしか値をとれない型である．論理型のことを $haskell.Bool$と書く．#footnote[Haskellではブール型を `Bool` と書く．]

もう一つ，計算機ならではの型がある．それは $haskell.Int$ とよく似ているが，特別に文字を扱うために考えられた#keyword[@char] $haskell.Char$ である．文字といってもその中身は整数である．整数ではあるが，わざわざ別な型とするのには理由がある．#footnote[Haskellでは $haskell.Char$ を ```haskell Char``` で表す．]
...
手続き型プログラミングを学習すると，逐次実行，条件分岐，繰り返しで任意のプログラムが書けることを教わるだろう．関数型プログラミングでは，逐次実行を意識することはあまりない．例外は入出力（IO）を扱う場合で，これは @ch-io で扱う．また我々は条件分岐について @ch-curry-style で見てきた．この節では繰り返し処理のための#keyword[@recursion]について学ぶ．
...
ここで改めて概念を整理しておこう．@tbl-functor-applicative-monad に示すように，関手型クラス $(haskell.Functor)$ に属する型はマップ演算子 $(ast.op.o)$ を持つ．アプリカティブ関手型クラス $(haskell.Applicative)$ に属する型はマップ演算子に加えてアプリカティブマップ演算子 $(ast.square)$ とピュア演算子 $(chevron.l square.filled chevron.r)$ を持つ．モナド型クラス $(haskell.Monad)$ に属する型は，マップ演算子，アプリカティブマップ演算子，ピュア演算子に加えて，バインド演算子 $(haskell.bind)$ を持つ．
...
do記法の利点のひとつに「継続渡しスタイル（continuation passing style）」を簡潔に書けることがある．継続渡しスタイルとは，関数に戻り値の返し先すなわち#keyword[@continuation]を引数として渡すスタイルである．@wikipedia-continuation-passing-style
