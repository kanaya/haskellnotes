...
プログラマが変数の値を変化させたい，つまり#keyword[@destructive-assignment]を行いたい理由はユーザ入力，ループ，例外，内部状態の変化，大域ジャンプ，継続を扱いたいからであろう．しかし，後に見るようにループ，例外，大域ジャンプに相当する操作に変数の破壊的代入は必要ない．ユーザ入力，内部状態の変化，継続に関しても章を改めて取り上げる．参照透過性を強くサポートするプログラミング言語をを#keyword[@functional-programming-language]と呼ぶ．
...
関数の正体は#keyword[@lambda-expression]である．ラムダ式とは，仮の引数をとり，その値をもとになにがしかの演算を行い，その結果を返す式である．ラムダ式は名前のない関数のようなものだ．それゆえ，無名関数と呼ばれることもある．
...
例えば $(1+)$ は $((+)1)$ と等価であり，和は#keyword[@commutative-law]を満たすので $(+1)$ とも等価である．ただし，マイナス演算子 $(-)$ だけは例外で，$(-1)$ はマイナス $1$ を表す．#footnote[Haskell は $(1+)$ を ```haskell (1+)``` と書く．また ```haskell (-1)``` はセクションではなくマイナス $1$ を表す（```haskell -1``` というリテラルとみなされる）．ただし ```haskell (- 1)``` のように空白を挟んでも同じくマイナス $1$ とみなされる（```haskell 1``` というリテラルに単項マイナス演算子が適用される）．]
