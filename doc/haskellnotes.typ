...
変数という呼び名に反して，変数の値は一度代入したら変えられない．そこで変数に値を代入するとは呼ばずに，変数に値を#keyword[束縛]するという． @eq-binding の右辺のように数式にハードコードされた値を#keyword[リテラル]と呼ぶ．
...
型については@ch-types で詳しく述べる．数学者は変数 $x$ が整数であることを $x in ZZ$ と書くが，本書では $x colon.double haskell.Int$ と書く．これは記号 $in$ を別の用途に用いるためである．また型の#keyword[注釈]と束縛をまとめて $x colon.double haskell.Int = 1$ と書く事もできる．#footnote[Haskellでは $x colon.double haskell.Int$ を ```haskell x :: Int``` と書き $x colon.double haskell.Int = 1$ を ```haskell x :: Int = 1 ``` と書く．]
...
@eq-composition-associativity で示される関係を#keyword[結合則]と呼ぶ．
...
@eq-composition-with-identity-function と @eq-composition-associativity をまとめて#keyword[関数の合成則]と呼ぶ．そこで，関数の合成則を箱に入れて掲載しておこう．
...
実は@eq-equiv は不完全なものである．このままでは型 $haskell.a$ に何の制約もないため，等値演算の定義されていない型が来るかもしれないからである．そこで，型自身が所属する，より大きな型があるとしよう．そのような抽象度の高い型を我々は#keyword[型クラス]と呼ぶ．例えば型 $haskell.Bool, haskell.Int,haskell.Integer, haskell.Float, haskell.Double$ は全て等値演算が定義できるので，型クラス $haskell.Eq$ に属すとする．この関係を我々は
...
演算子 $*_"M"$ は#keyword[アクション]版のマップ演算子である．アクションについては @ch-monad で扱う．
...
手続き型プログラミングを学習すると，逐次実行，条件分岐，繰り返しで任意のプログラムが書けることを教わるだろう．関数型プログラミングでは，逐次実行を意識することはあまりない．例外は入出力（IO）を扱う場合で，これは @ch-io で扱う．また我々は条件分岐について @ch-curry-style で見てきた．この節では繰り返し処理のための#keyword[再帰]について学ぶ．
...
コンテナ変数 $u_*, v_*$ のいずれかが $nothing.rev$ であれば式全体の値が $nothing.rev$ になる．これは2個の計算を並列に行って，その結果をそれぞれ $u_*, v_*$ に入れておき，最後に関数 $omega$ に投げるという#keyword[計算構造]を具現化したものである．#footnote[@eq-applicative-style-psi-phi はHasellのdo記法を用いると $w_* = haskell.kwdo { u <- phi x; v <- psi y; chevron.l omega u v chevron.r } $ と書くことが出来る．]
