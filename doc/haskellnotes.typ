...
型 $haskell.a$ から型 $[haskell.a]$ を生成する演算子を#keyword[リスト型コンストラクタ]と呼んで $[space.thin]$ と書く．型 $[haskell.a]$ は $haskell.typeconstructor1([space.thin], haskell.a)$ のシンタックスシュガーである．#footnote[Haskellでは $haskell.typeconstructor1([space.thin], haskell.a)$ を ```haskell [] a``` と書く．これは ```haskell [a]``` と同じことである．]
...
#figure(
  ...
  table(
    ...
    [$[haskell.a]$ または $haskell.typeconstructor1([space.thin], haskell.a)$], $[space.thin]$, $*$, $[x]$,
...
型 $haskell.a$ のリストの変数は $x_"s" colon.double [haskell.a]$ という型注釈を持つ．これは $x_"s" colon.double haskell.typeconstructor1([space.thin], haskell.a)$ のシンタックスシュガーである．#footnote[Haskellでは ```haskell xs :: [] a``` と書く．]
