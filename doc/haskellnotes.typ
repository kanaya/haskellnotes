...
上記の例のように，数学における関数合成をそのままプログラミングに持ち込めれば，部分プログラムの合成も見通しが良いものになる．このような考え方が「関数型プログラミング」のエッセンスなのである．@bryan-osullivan @graham-hutton @miran-lipovaca @richard-bird @will-kurt
...
いま「世界」と呼んだものを，数学者は#keyword[@category-mathematical]と呼ぶ．圏とは#keyword[@object]と#keyword[@morphism]の組み合わせである．本書では「対象」とは変数のことであり，射とは関数だと思えば良い．そして，圏から圏へとジャンプさせるもの $(~~>)$ を#keyword[@functor-mathematical]と呼ぶ．この例で言えば値コンストラク $[x], haskell.Just(x)$ とマップ演算子 $*, ast.op.o_?$ が関手である．値コンストラクタ $haskell.Just(x)$ は $haskell.a -> haskell.MaybeA$ という型を持ち，マップ演算子 $ast.op.o_?$ は $(haskell.a -> haskell.b) -> (haskell.MaybeA -> haskell.MaybeB)$ という型を持つ．
...
実用的なプログラムには必ず#[@io]（IO）がある．ところがIOとは参照透過性を持たない行動であり，関数型プログラミングの世界観とは相容れない．そこで，HaskellではIOをモナド型クラスで表現する．IOという「破壊活動」をモナド型クラスのインスタンスの変数に閉じ込めて，プログラムの他の部分と分離するのである．#footnote[Haskellでは「型クラス（type class）のインスタンス（instance）」は型（type）のことである．C++やJavaではインスタンスがオブジェクト（object）を意味するので，混同しないように気をつけよう．]
